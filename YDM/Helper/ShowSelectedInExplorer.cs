...
namespace SHOpenFolderAndSelectItems
{
    ...
    static class ShowSelectedInExplorer
    {
        ...
        static IntPtr GetShellFolderChildrenRelativePIDL(IShellFolder parentFolder, string displayName)
        {
            ...
            parentFolder.ParseDisplayName(IntPtr.Zero, null, displayName, out var pchEaten, out var ppidl, ref pdwAttributes);
        ...
        {
            return GetShellFolderChildrenRelativePIDL(NativeMethods.SHGetDesktopFolder(), path);
        }

        static Guid _iID_IShellFolder = typeof(IShellFolder).GUID;

        static IShellFolder PIDLToShellFolder(IShellFolder parent, IntPtr pidl)
        {
            var result = parent.BindToObject(pidl, null, ref _iID_IShellFolder, out var folder);
